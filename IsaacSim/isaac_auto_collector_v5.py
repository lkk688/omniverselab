...
SUPPORTED_ENVS: dict[str, EnvPreset] = {
    ...
    "stack-ik-rel": EnvPreset(
        ...
        hover_height=0.10,
        grasp_height=0.0,
        lift_height=0.18,          # moderate lift for stacking (not too high)
        place_approach_height=0.12, # lower transit → faster stacking
        place_release_height=0.03,  # release just 3cm above cube_2 top
        grasp_settle_frames=50,    # 1.0s grip time (was 70=1.4s)
        min_cube_lift_z=0.06,
    ...
    "open-drawer": EnvPreset(
        ...
        # Pitch=π/2 (horizontal gripper) is applied starting from DESCEND phase only.
        # During APPROACH_XY / HOVER / OPEN_GRIP pitch correction is intentionally
        # disabled (apply_pitch_roll=False) so the IK can freely choose arm config.
        target_pitch=1.5708,              # π/2 — horizontal gripper
        handle_body="drawer_handle_top",  # articulation body to track
        # Distances along approach_dir (-X = toward robot).
        # Keep approach_height large so the arm has room to settle freely first.
        # hover_height small so DESCEND only has ~8 cm of insertion travel.
        approach_height=0.20,   # stand 20 cm in front of handle
        hover_height=0.08,      # hover 8 cm in front (arm already extended, less insert travel)
        grasp_height=0.0,       # grip at handle centre
        lift_height=0.22,       # pull 22 cm back toward robot
        # Verification: check handle moved ≥ 6 cm, not cube Z height
        verify_min_pull=0.06,
        min_cube_lift_z=-999.0, # unused when verify_min_pull > 0
        # Controller tuning — slower descent for precision; more frames for reach
        grasp_settle_frames=60,
        max_descend_frames=200,  # more time for orientation+position to converge
        descend_max_action=0.20,
        kp_far=5.0, kp_near=2.5,
        xy_align_threshold=0.015,
        z_align_threshold=0.04,  # generous: convergence check for 3-D distance
        max_retries=3,
    ),
}
...
class PickAndPlaceController:
    ...
    def __init__(self, preset: EnvPreset, device: str,
        ...
        # True when target_pitch != 0 (e.g. drawer: horizontal gripper).
        # When True, pitch/roll correction is enabled in ALL phases including
        # APPROACH/HOVER/OPEN_GRIP, so the arm orients early.
        self._needs_early_pitch = bool(abs(preset.target_pitch) > 0.1)
    ...
    def _action(self, tcp_target, target_yaw, ee_pos, ee_quat,
        ...
        apply_pitch_roll=False: skip roll/pitch correction so the IK solver can freely
                               choose arm configuration — used during early approach phases
                               where a large orientation correction (e.g. pitch=π/2 for
                               drawer) would otherwise fight position convergence and lock
                               the arm in a local minimum far from the target.
        """
        if max_act is None:
        ...
        # Orientation correction — only apply when requested.
        # During APPROACH_XY / HOVER / OPEN_GRIP we skip pitch/roll so the IK
        # solver can freely choose an elbow configuration that maximises reach.
        ...
        # else: act[3] = act[4] = 0  (already zeros; arm orientation unconstrained)
    ...
    def compute(self, obj_pos, obj_quat, ee_pos, ee_quat,
                place_obj_pos=None, place_obj_quat=None):
        """
        ...
        Args:
            ...
            place_obj_quat: quaternion of the place-target object (for yaw alignment)
        ...
        # Yaw to use during placement: match the place target's orientation
        place_yaw = get_yaw(place_obj_quat) if place_obj_quat is not None else target_yaw
        is_terminal = False
        ...
        # NOTE: apply_pitch_roll is disabled during early approach UNLESS the
        # task requires non-zero target_pitch (e.g. drawer: horizontal gripper).
        # For such tasks, pitch correction must start early so the arm orients
        # before reaching the target — otherwise it gets stuck.
        _early_pitch = self._needs_early_pitch
        if self.phase == self.APPROACH_XY:
            ...
            act, dxy, dz, dyaw = self._action(tgt, target_yaw, ee_pos, ee_quat,
                                              grip=1.0, apply_pitch_roll=_early_pitch)
        ...
        # ── MOVE_TO_TARGET (basket / stack transit) ───────────────────────────
            ...
            act, dxy, dz, _ = self._action(tgt, place_yaw, ee_pos, ee_quat,
                                           grip=-1.0)
        ...
        # ── DESCEND_TO_TARGET ─────────────────────────────────────────────────
            ...
            act, _, dz, _ = self._action(tgt, place_yaw, ee_pos, ee_quat,
                                         grip=-1.0,
                                         max_act=p.descend_max_action)
        ...
        # ── RELEASE ───────────────────────────────────────────────────────────
            ...
            tgt = self.place_target.clone()
            tgt[2] += self.p.place_release_height
            act, *_ = self._action(tgt, place_yaw, ee_pos, ee_quat, grip=1.0)
...
def main():
    ...
    if preset.handle_body:
        ...
        try:
            ...
            print(f"    Body names: {getattr(_pick_entity.data, 'body_names', 'n/a')}")
    ...
    while simulation_app.is_running():
        ...
        if use_place and preset.place_key:
            ...
            place_obj_quat = env.scene[preset.place_key].data.root_quat_w[0]
        ...
        if state == "SEMI_HOVER":
            ...
            if saver.total_demos >= max_demos:
                ...
                auto_act, _, done = controller.compute(
                    pick_pos, pick_quat, ee_pos, ee_quat,
                    place_obj_pos, place_obj_quat)
                final_action = auto_act
                if done:
